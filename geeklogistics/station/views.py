    ...
    try:
        ...
        try:
            orders = Order.objects.filter(
                Q(order_status=0, poi_nearest=station_id, status='0') | Q(order_status=100, poi_nearest=station_id,
                                                                          status='0'))
            limit = 20
        ...
        reminder = "该配送站不存在"
    return render_to_response('station/order_list.html',
                              {'current_url': 'area', 'orders': orders, 'start_index': start_index, 'js_url': js_url,
                               'reminder': reminder})
