...
class StatusRecord(models.Model):
    ...
    def record_text(self):
        ...
        # todo：加入出库入库等提示？
        if (self.status == 200):
            text = time_format + ' ' + '配送员 ' + operator.name.encode('utf-8') + '(电话：' + operator.phone.encode(
                'utf-8') + ') 已从商家取货';
        elif (self.status == 300):
            if self.operator_type == '0':
                if self.operator_id == 11:
                    text = time_format + ' ' + '货物正在分拣站 ' + operator.name.encode(
                        'utf-8') + '(电话：' + operator.phone.encode('utf-8') + ') 进行分拣';
                else:
                    text = time_format + ' ' + '货物已经到达 ' + operator.name.encode(
                        'utf-8') + '(电话：' + operator.phone.encode('utf-8') + ')';
            elif self.operator_type == '2':
                text = time_format + ' ' + '货物正由司机 ' + operator.name.encode('utf-8') + '(电话：' + operator.phone.encode(
                    'utf-8') + ') 运往下一站点';
        elif (self.status == 400):
            text = time_format + ' ' + '货物正由配送员 ' + operator.name.encode('utf-8') + '(电话：' + operator.phone.encode(
                'utf-8') + ') 开始配送';
